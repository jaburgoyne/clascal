...
c
      common /cstak/ dstak
c
      double precision dstak(100000000)
      real rstak(200000000)
      integer istak(200000000)
c
...
c this subroutine sets up convergence criteria
 ...
 1000 format(0x,5x,"s start=",1x,f5.1)
 ...
 1001 format(0x,5x,"torgerson start")
 1002 format(0x,5x,"random start")
       write(lp,2000) itmax,itxmax,itwmax,conv,wconv
 2000  format(/0x,5x,'max it for mstp:',i3,3x,'max it for xstp:',i3,3x,
     1     'max it for xstp:',i3,/0x,5x,'mstp conv crit:',f8.5,
...
c        if(pgword)write(lp,4327)
 ...
 4327   format(//0x,5x,'Results of Torgerson Start')
c	if(pgword)print 6667,cmax
c 6667 format(10x,21hadditive constant is ,f10.3)
	do 77 i=1,nstim
		do 78 j=1,nstim
...
c   do eigenanalysis
 ...
 2010 format(0x,5x,"eigenvalues of t-soln")
 2011 format(0x,5x,5(1x,f10.3))
      do 90 iq=1,ndim
	ahmax=0.d0
	do 96 iqq=1,nstim
...
c 8001       format(10x,3hijx,2x,i3,2x,i3,2x,f6.3,2x,f6.3)
...
* and s's independent of l1
 ...
 1000 format(0x,5x,"distance neg s+s,d",2(1x,f20.10))
      dstar=sqrt(dstar)
	return
	end
...
c
 ...
 10       continue
     ...
     1      wmean,mean,nspr,nmaxw,y,sig)
        ...
        double precision dstak(100000000),sum
        real rstak(200000000)
        integer istak(200000000) 
        equivalence(dstak(1),rstak(1))
        equivalence(dstak(1),istak(1))
...
c 1718  format(0x,5x,"nsd,ns,nmodel,nmax,nspace   ",5(i3,1x))
...
c 1050 format(10x,14hgradient check,"in dimstp")
...
c       print 1717
 ...
 1001 format(0x,10x,5hgnorm,1x,f15.5,2x,6hgnormx,1x,f15.5)
 1000 format(0x,5x,6hitx.no,1x,i3,2x,2hss,f10.3)
...
c  compute initial reduced gradient and hessian store pos coeff in free
 ...
 1816 format(0x,5x,"ifree",36(1x,i2))
        nfrees=nsd
        do 202 in=1,nss
 ...
 1014     format(0x,5x,"reduced grnorm=",1x,f20.10)
        end if
      else
        nfrees=nsd+ns
        call vecex(nspace,1,1,1,1,g,ghat,1.d0)
...
c   compute search direction
 ...
 2001 format(10x,16her from dml i1r=,i2)
...
c   do linesearch
 ...
 1607 format(0x,"linesearch output,sso,ssn",2(1x,f20.10))
  ...
  841      continue
   ...
   85 if(pgword)print 1004,itxs,ind,ssn
      if(pgword)print 1001,gnorm,gnormx 
 1004 format(0x,1x,"itx no ",1x,i3,1x,"ind",1x,i1,
     11x,"ss",1x,f10.3)
      if(ssn.le.sso)go to 90
      ssn=sso
      if(pgword)print 1007
 1007 format(0x,5x,21hfun not imp this step)
      go to 100
 ...
 1002 format(0x,10x,14hconfig converg)
...
c      do 6 i=1,nw
c    6 print 1051,su(i),g(i)     
c 1050 format(10x,14hgradient check,"inwstp")
c 1051 format(5x,2hs=,f20.5,5x,2hg=,f20.5)
c   *******************
c   *******************
      if(abort)go to 98
 ...
 1000 format(0x,5x,6hitw.no,1x,i3,2x,2hss,f20.3)
...
c   compute search direction
        ...
        call dmlss(ahe,nfrees,irank,c1,0,b1,ier)
        if(ier.ne.0)print 2001,ier
 2001 format(10x,16her from dml i2r=,i2)
...
c   do linesearch
 ...
 1004 format(0x,1x,"itw no ",1x,i3,1x,"ind",1x,i1,
     11x,"ss",1x,f10.3)
      if(ssn.le.sso)go to 90
      ssn=sso
      if(pgword)print 1007
...
c 6111 format(10x,3hd<0,2x,4hi,j=,i3,i3,2x,2hl=,i3,2x,2hd=,f20.10)
c      else
c	continue
c      end if	
      dhat=dstar
...
c
      double precision dstak(100000000)
      real rstak(200000000)
      integer istak(200000000),mem(nsub)
c
      equivalence(dstak(1),rstak(1))
      equivalence(dstak(1),istak(1))
c
...
c  using model parameters generate model distances for each class
     ...
     1     x,w,s,rstak(jdtmp))
       do 40 irep=1,nrep
c       print 9898,irep
 9898  format(/0x,5x,"replication no : ",i3)
...
c  first step set to trial
 ...
 1000 format(0x,5x,"linesearch",2x,i3,3(1x,f12.5))
...
c  main loop for search
 ...
 1419 format(0x,5x,"stepsize 0 or neg =",1x,f10.3)
       f=fo
...
c  step is reduced by factor bb
 ...
 1417 format(0x,5x,"fcn is worse ..step is halved")
...
c  test1 means successful convergence
 ...
 1607 format(0x,5x,"linesearch convergence,fcn=",1x,f20.10)
      trial=a4
      ind=0
      return
...
c  current slope positive
 ...
 1418 format(0x,5x,"current slope is pos")
      ips=1
...
c   initialize loglikelihood
 ...
 1000 format(//0x,5x,"iter no.",3x,i3,3(3x,3e15.7))
...
c      if(sngl((flglik-flglo)).ge.conv)go to 500
 ...
 5053  format(/0x,5x,"max no. of m-step iterations")
       go to 700
  600  continue
        if(nrep.lt.20)print 5052
 5052  format(/0x,5x,"mstep convergence")
  700  call center(x,nstim,ndim)
 5050 format(0x,5x,"x after m step")
 5051 format(0x,5(1x,f15.4))
...
c  this subroutine prints the output
 ...
 3000 format(//0x,5x,'w')
   ...
   54        continue
   53          continue
 4000      format(0x,5x,"dimension no.:",i2,3x,"class no:",i2,
     1          3x,"weight:",e15.7)         
      else
       continue
      end if 
